...
# return classifier without pre-training
def get_clf(arch, num_classes=1000):

    if arch in ['resnet18', 'resnet50']:
        model = timm.create_model('%s.tv_in1k' % arch, pretrained=False, num_classes=num_classes)
    elif arch in ['vit_small', 'vit_base']:
        model = timm.create_model('%s_patch16_224.augreg_in1k' % arch, pretrained=False, num_classes=num_classes)
    else:
...
def get_pclf(arch, pretrain, num_classes=1000):
    ...
    if pretrain in ['weakly', 'fully']:
        ...
        if arch == 'resnet18':
            model = timm.create_model('hf_hub:krishanwalia30/resnet18-random-classifier-by-krishan', pretrained=True, num_classes=num_classes)
            model.default_cfg['input_size'] = (3, 256, 256)
        else:
            raise Exception('ARCH NOT SUPPORTED')
